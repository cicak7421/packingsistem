...
## Modul Inventory Stock (baru)

Halaman **`inventory.html`** buat kelola stok produk, terpisah dari alur packing tapi terhubung otomatis ke sana.

**Cara aktifin:**
1. Buka **SQL Editor** di Supabase project yang sama → jalankan isi file `migration-inventory.sql` (bikin tabel `products`, `stock_log`, role baru `inventory`, kolom `users.permissions`, dan bucket Storage `product-images`).
2. Redeploy (kalau perlu) — gak ada environment variable baru yang wajib ditambah, karena upload gambar produk lewat backend yang sudah pakai `SUPABASE_SERVICE_ROLE_KEY`.
3. Login sebagai admin → menu **Kelola Akun** → bikin akun baru dengan role **Staff Inventory**, atau tetap pakai akun `packing`/`admin` yang sudah ada (mereka otomatis bisa lihat tab **Inventory**).

**Fitur:**
- Tampilan grid produk (gambar, SKU, kategori, stok) yang auto-refresh tiap 5 detik (polling ringan, bukan koneksi realtime terus-menerus) — cukup buat kebutuhan "cek stok real-time" tanpa nambah kompleksitas infra.
- Tombol **+ Tambah** / **− Kurang** stok per produk, dengan catatan opsional; setiap perubahan tercatat di riwayat (klik gambar produk buat lihat riwayat lengkap).
- Upload gambar produk (maks 4MB) langsung ke Supabase Storage lewat backend.
- **Auto-kurangi stok otomatis**: begitu tim packing scan barcode & pesanan ditandai `sudah_packing`, sistem otomatis cocokkan SKU pesanan itu ke tabel `products` dan mengurangi stoknya (dicatat sebagai riwayat "Otomatis (Packing)"). SKU yang belum terdaftar di Inventory dilewatin diam-diam, gak bikin proses scan gagal.
- **Sub-akun & izin bisa diatur admin**: role baru `inventory` (staff gudang) selain `admin`/`cs`/`packing` yang sudah ada. Di menu **Kelola Akun**, tiap akun (selain admin) punya tombol **"Atur Akses"** buat toggle 4 izin granular: Lihat Stok, Tambah/Kurangi Stok, Kelola Produk, Kelola Sub-Akun — jadi admin bisa kasih akses beda-beda per staff sesuai kebutuhan (misal staff packing cuma boleh lihat stok, gak boleh edit produk).

**Batasan yang perlu diketahui:**
- Auto-kurangi stok mencocokkan berdasarkan **SKU** di kolom `orders.sku` (hasil import) ke `products.sku` — pastikan SKU yang didaftarkan di Inventory sama persis dengan SKU yang dipakai di file export marketplace.
- Refresh grid pakai polling tiap 5 detik (bukan WebSocket/Supabase Realtime) — cukup real-time buat kebutuhan operasional harian; kalau butuh lebih instan (sub-detik) itu bisa di-upgrade nanti ke Supabase Realtime channel.

### Lokasi Rak (tracking barang ditaro di rak mana)

Tambahan lagi: jalankan **`migration-inventory-rak.sql`** (setelah `migration-inventory.sql`) buat aktifin fitur ini.

- Kode rak bebas format ("Rak A1", "T01", dst) — dikelola sendiri lewat tombol **🗂 Kelola Rak** di halaman Inventory, bisa ditambah/dihapus kapan aja.
- Satu produk **bisa kesebar di beberapa rak sekaligus** (misal 60 pcs di Rak A1, 40 pcs di T01) — ditampilkan sebagai badge kecil di tiap kartu produk.
- Pas nambah/kurangi stok, ada pilihan **"Masuk ke rak"** / **"Ambil dari rak"** (opsional) — kalau dipilih, breakdown lokasi ikut ke-update otomatis bareng total stok.
- Kalau ada stok yang belum "ditempatkan" secara spesifik (total stok lebih besar dari total yang sudah dialokasikan ke rak), itu ditandai badge kuning **"Belum ditempatkan"** — tinggal klik **🗂 Lokasi Rak** di kartu produk buat lihat detail & **pindahkan stok antar rak** kapan aja (tanpa mengubah total stok produk).
- Filter **"Semua Rak"** di toolbar dipakai buat cari produk mana aja yang disimpan di satu rak tertentu (klik rak-nya).
