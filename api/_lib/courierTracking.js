...
// sama sekali -- paket langsung dijemput driver begitu di-packing, jadi gak akan pernah ada
// update resi yang bisa dicek otomatis. Buat kurir-kurir ini, "sudah packing" DIANGGAP
// otomatis "sudah dikirim" (lihat pemakaiannya di api/_lib/routes/packing.js, saat scan).
// Dicocokkan dari teks "Opsi Pengiriman" hasil export marketplace, case-insensitive, pakai
// kombinasi kata kunci (bukan exact match) biar tetap kena walau redaksinya dikit beda --
// tapi tetap dijaga supaya SPX/GoSend versi REGULER (non-instant, yang masih bisa dicek
// BinderByte) gak ikut ke-bypass.
function isInstantCourier(opsiPengiriman) {
  const text = String(opsiPengiriman || '').toLowerCase();
  if (!text) return false;
  if (text.includes('grabexpress') && text.includes('instant')) return true; // GrabExpress Instant
  if (text.includes('gosend') && text.includes('instant')) return true; // GoSend Instant Prioritas
  if (text.includes('gojek')) return true; // Gojek, Gojek Instant
  if (text.includes('spx') && (text.includes('instant') || text.includes('sameday'))) return true; // SPX Instant, SPX Instant Prioritas, SPX Sameday
  return false;
}
