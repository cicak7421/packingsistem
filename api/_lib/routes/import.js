...
// Pesanan dianggap "udah diproses" (gak boleh ketiban update dari import lagi) kalau
// sudah packing ATAU status resinya udah dikirim/diterima. Dipakai buat nolak duplikat yang
// beresiko: kalau tim udah packing/kirim berdasarkan data lama, terus ada file lain nyusul
// bawa No. Pesanan yang sama dengan data beda (alamat/SKU/dll ketuker atau kekoreksi di
// marketplace setelahnya), re-import BISA nimpa data yang udah dipakai buat packing fisik --
// gak nyambung lagi sama paket yang sudah beneran dikemas/dikirim. Pesanan yang masih
// Belum Packing tetap boleh diupdate seperti biasa (itu justru gunanya merge/upsert -- benerin
// No. Resi dkk yang nyusul sebelum sempat diproses).
function isAlreadyProcessed(existingRow) {
  if (!existingRow) return false;
  if (existingRow.status_packing === 'sudah_packing') return true;
  if (existingRow.status_resi === 'dikirim' || existingRow.status_resi === 'diterima') return true;
  return false;
}
...
router.post('/', requireAuth, requireRole('admin', 'cs'), upload.single('file'), async (req, res) => {
  ...
  // Pisahin baris yang boleh diupsert vs yang ditolak karena pesanannya udah diproses
  // (sudah packing/dikirim/diterima) -- lihat isAlreadyProcessed() di atas. Yang ditolak
  // SAMA SEKALI gak disentuh (data lama di database dibiarkan apa adanya), bukan cuma
  // dilewatin sebagian field-nya kayak mergeWithExisting() buat yang masih belum diproses.
  const ditolakSudahDiproses = [];
  const toUpsertFiltered = toUpsert.filter((row) => {
    const existingRow = existingByNoPesanan.get(row.no_pesanan);
    if (isAlreadyProcessed(existingRow)) {
      ditolakSudahDiproses.push(row.no_pesanan);
      return false;
    }
    return true;
  });

  const mergedToUpsert = toUpsertFiltered.map((row) => {
    const existingRow = existingByNoPesanan.get(row.no_pesanan);
  ...
  res.json({
    ...
    skipped_baris_tanpa_no_pesanan: skipped,
    ditolak_sudah_diproses: ditolakSudahDiproses.length,
    ditolak_sudah_diproses_list: ditolakSudahDiproses.slice(0, 50),
  });
});
...
router.post('/manual', requireAuth, requireRole('admin', 'cs'), async (req, res) => {
  ...
  // Sama kayak import file: pesanan yang udah diproses (sudah packing/dikirim/diterima)
  // ditolak, gak boleh ketiban data baru dari form ini -- lihat isAlreadyProcessed() di atas.
  if (isAlreadyProcessed(existing)) {
    return res.status(409).json({
      error: `Pesanan ${noPesanan} sudah ${existing.status_packing === 'sudah_packing' ? 'di-packing' : 'dikirim/diterima'}, gak bisa ditimpa lagi lewat sini. Kalau memang perlu dikoreksi, ubah datanya langsung lewat menu yang sesuai (misal "Di-packing Oleh" di Dashboard).`,
