...
// Bypass otomatis buat kurir instant/sameday (lihat isInstantCourier() di courierTracking.js
// buat daftar lengkapnya) -- pesanan dengan kurir ini SELALU dianggap langsung "sudah packing"
// + "sudah dikirim" begitu masuk ke sistem (baik lewat import file maupun Input Manual), TANPA
// perlu discan tim packing atau nunggu cek resi BinderByte dulu. Ini nutupin kasus paket
// instant yang emang gak sempat/gak perlu di-scan (driver langsung jemput di tempat), yang
// sebelumnya bikin pesanan-pesanan ini nyangkut lama di "Menunggu Packing" padahal
// kenyataannya udah lama dikirim/diterima.
//
// SENGAJA gak nimpa pesanan yang statusnya udah lebih maju dari itu (misal sudah pernah
// discan manual sama tim packing, atau sudah ditandai diterima) -- cuma dipakai buat
// "mendorong maju" pesanan yang masih ketinggalan di belakang, bukan buat mundurin status
// yang udah lebih baik.
function applyInstantCourierBypass(row, existingRow) {
  if (!isInstantCourier(row.opsi_pengiriman)) return row;
  if (existingRow && existingRow.status_packing === 'sudah_packing') return row; // udah diproses, jangan diutak-atik lagi
  return {
    ...row,
    status_packing: 'sudah_packing',
    status_resi: 'dikirim',
    dikirim_at: row.waktu_pesanan_at || existingRow?.waktu_pesanan_at || new Date().toISOString(),
  };
}
...
router.post('/', requireAuth, requireRole('admin', 'cs'), upload.single('file'), async (req, res) => {
  ...
  const mergedToUpsert = toUpsert.map((row) => {
    ...
    return applyInstantCourierBypass(mergeWithExisting(row, existingRow), existingRow);
  });
...
router.post('/manual', requireAuth, requireRole('admin', 'cs'), async (req, res) => {
  ...
  const finalRow = applyInstantCourierBypass(mergedRow, existing);

  const { error } = await supabase.from('orders').upsert(finalRow, { onConflict: 'no_pesanan' });
  if (error) {
    console.error(error);
