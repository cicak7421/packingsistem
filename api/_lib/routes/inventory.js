...
router.get('/products', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
router.get('/summary', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
// ---------- Detail 1 produk ----------
router.get('/products/:id', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
router.get('/products/:id/logs', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
router.get('/stock-chart', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
router.get('/categories', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
// ---------- Master lokasi rak ----------
router.get('/rak-lokasi', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
...
router.delete('/rak-lokasi/:id', requireAuth, requireRole('admin', 'inventory'), requirePermission('manage_products'), async (req, res) => {
  ...
  // Bersihin row product_rak yang qty-nya udah 0 buat rak ini SEBELUM hapus rak-nya --
  // kalau gak, DELETE di bawah bakal tetap gagal kena FK constraint (product_rak.rak_id
  // "on delete restrict"), soalnya row qty=0 itu masih ada / masih nunjuk ke rak ini
  // walau gak keitung "masih dipakai" oleh cek di atas.
  await supabase.from('product_rak').delete().eq('rak_id', req.params.id);
...
router.get('/products/:id/rak', requireAuth, requireRole(...VIEW_ROLES), requirePermission('view_stock'), async (req, res) => {
  const { data: product, error: findErr } = await supabase.from('products').select('*').eq('id', req.params.id).maybeSingle();
  if (findErr || !product) return res.status(404).json({ error: 'Produk tidak ditemukan' });
  const [withRak] = await attachRakBreakdown([product]);
  res.json({ product: withRak });
