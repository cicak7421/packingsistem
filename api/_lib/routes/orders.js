...
router.patch('/:id/packed-by', requireAuth, requireRole('admin', 'cs'), async (req, res) => {
  ...
  // kekoreksi juga, gak cuma tampilan "Di-packing Oleh" doang. scan_log cuma nyimpen 1 baris
  // result='ok' per pesanan (lihat komentar di packing.js), jadi aman dicari via order_id+result
  // tanpa ambigu. Kalau baris scan_log-nya emang belum ada (pesanan auto-marked, gak pernah
  // discan asli lewat menu Scan Packing) -- sengaja DIBIARKAN, karena orang yang dipilih di sini
  // gak beneran melakukan scan fisiknya, jadi wajar kalau gak nambah hitungan performa dia.
  if (packerId !== null) {
    const { error: scanSyncErr } = await supabase
      .from('scan_log')
      .update({ user_id: packerId })
      .eq('order_id', order.id)
      .eq('result', 'ok');
    if (scanSyncErr) console.error('Gagal sinkron scan_log setelah koreksi packed_by:', scanSyncErr.message);
  }
