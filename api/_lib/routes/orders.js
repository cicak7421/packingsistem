...
// Sengaja juga dipakai buat "ngasih kredit" performa packing ke packer yang dipilih manual di
// sini walau pesanannya gak pernah discan asli -- lihat sinkronisasi scan_log di bawah.
router.patch('/:id/packed-by', requireAuth, requireRole('admin', 'cs'), async (req, res) => {
  ...
  const { data: order, error: findErr } = await supabase
    .from('orders')
    .select('id, packed_at, status_packing, no_resi, no_pesanan')
    .eq('id', req.params.id)
    .maybeSingle();
  ...
  // Ikut sinkronin scan_log biar rekap "Performa Packing" (dihitung dari scan_log, BUKAN dari
  // orders.packed_by -- lihat get_packing_performance() di migration-performance.sql) ikut
  // kekoreksi juga, gak cuma tampilan "Di-packing Oleh" doang.
  if (packerId !== null) {
    // Coba update dulu baris scan_log 'ok' yang udah ada buat pesanan ini (kasus koreksi salah
    // orang dari scan asli). scan_log cuma nyimpen 1 baris result='ok' per pesanan (lihat
    // komentar di packing.js), jadi aman dicari via order_id+result tanpa ambigu.
    const { data: syncedRows, error: scanSyncErr } = await supabase
      .from('scan_log')
      .update({ user_id: packerId })
      .eq('order_id', order.id)
      .eq('result', 'ok')
      .select('id');
    if (scanSyncErr) {
      console.error('Gagal sinkron scan_log setelah koreksi packed_by:', scanSyncErr.message);
    } else if (!syncedRows || syncedRows.length === 0) {
      // Gak ada baris buat di-update -- pesanan ini emang belum pernah discan asli (auto-marked
      // lewat import/cek resi). Tetap dibikinin baris scan_log baru, biar packer yang dipilih
      // admin/CS di sini ikut kehitung di Performa Packing juga (permintaan: "meski gak discan,
      // kalau diubah sama admin ikut ke ubah performanya"). Waktunya dipakein packed_at (bukan
      // waktu sekarang), biar jatuh di tanggal yang sama kayak "Waktu Packing" yang ditampilkan.
      const { error: scanInsertErr } = await supabase.from('scan_log').insert({
        order_id: order.id,
        user_id: packerId,
        scanned_value: updated.no_resi || updated.no_pesanan || String(order.id),
        result: 'ok',
        created_at: updated.packed_at || new Date().toISOString(),
      });
      if (scanInsertErr) console.error('Gagal bikin scan_log baru setelah set packed_by manual:', scanInsertErr.message);
    }
  }
