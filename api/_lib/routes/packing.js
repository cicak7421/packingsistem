...
// ---------- Integrasi Inventory: auto-kurangi stok saat pesanan selesai di-packing ----------
// Kolom orders.sku formatnya bisa:
//   - "SP30S x20, SP74 x3, SP35L x2"  (pesanan multi-item, dari import format multi-platform)
//   - "SP30S"                          (pesanan 1 item, pakai orders.jumlah sebagai qty-nya)
// Fungsi ini mem-parse itu, lalu cari produk yang SKU-nya cocok persis (case-insensitive) di
// tabel `products` dan mengurangi stoknya + mencatat ke stock_log (source: 'auto_packing').
// SKU yang gak ketemu produknya dilewatin diam-diam (bukan error) -- wajar kalau belum semua
// SKU didaftarkan ke modul Inventory.
//
// Varian bundling "PKT": SKU pesanan bisa berupa "PKTxx-KODE", contoh "PKT20-SP24" artinya
// 1 paket isi 20pcs SKU "SP24". Kalau ketemu pola ini, SKU yang dicari ke tabel `products`
// adalah SKU aslinya ("SP24"), dan qty yang dikurangi dikali jumlah pcs per paket (20) --
// dikombinasikan juga sama qty "xN" kalau ada, misal "PKT20-SP24 x3" = 3 paket = 60pcs.
...
async function autoDeductStockForOrder(order, userId) {
  const lines = parseSkuLines(order);
  ...
  for (const line of lines) {
    ...
    if (!product) { notFound.push(line.sku); continue; }

    const newStock = Math.max(0, product.stock_qty - line.qty); // gak sampai minus di database
    await supabase.from('products').update({ stock_qty: newStock }).eq('id', product.id);
    // Auto-deduct ini gak tau produk diambil dari rak mana -- biar breakdown per-rak gak
    // "nyangkut" nunjukin stok lebih dari total yang sebenarnya, trim breakdown-nya juga.
    ...
    await supabase.from('stock_log').insert({
      ...
      source: 'auto_packing',
      note: `Otomatis dari packing pesanan ${order.no_pesanan}`,
      order_id: order.id,
      user_id: userId,
    });
  }

  return notFound.length ? { skus_not_registered: notFound } : null;
}
...
router.post('/scan', requireAuth, requireRole('packing', 'admin'), async (req, res) => {
  ...
  if (order.status_packing === 'sudah_packing') {
    // Sengaja TIDAK ditulis ke scan_log. Duplikat scan (paket yang sama discan ulang,
    // salah ambil, dobel packing man, dll) bisa kejadian berkali-kali per paket dan gak
    // ada nilai historisnya -- yang penting cuma notifikasi real-time "udah dipacking
    // sama siapa", dan itu sudah kejawab dari kolom orders.packed_by di bawah, gak perlu
    // nyimpen baris log terpisah tiap kali ke-scan ulang. Kalau ini tetap dicatat, dengan
    // volume ratusan ribu paket, jumlah baris scan_log bisa berkali-kali lipat dari jumlah
    // paket beneran (tiap paket bisa kena scan ulang berkali-kali oleh beberapa orang).
  ...
  // scan_log sekarang HANYA diisi untuk scan yang beneran berhasil (result 'ok') -- jadi
  // baris di tabel ini kira-kira 1:1 sama jumlah paket yang sudah packing, bukan tiap kali
  // ada percobaan scan. Ini yang dipakai buat "Riwayat scan hari ini" & rekap Performa Packing.
  await supabase.from('scan_log').insert({ order_id: order.id, user_id: req.user.id, scanned_value: scannedValue, result: 'ok' });

  // Auto-kurangi stok inventory berdasarkan SKU pesanan ini (kalau SKU-nya cocok sama
  // produk yang terdaftar di modul Inventory). Sengaja dibungkus try/catch terpisah --
  // kalau produknya belum terdaftar / SKU kosong / stok kurang, JANGAN sampai bikin proses
  // packing gagal, cukup dilewatin diam-diam (staf inventory tetap bisa adjust manual nanti).
  let stockWarning = null;
  try {
    stockWarning = await autoDeductStockForOrder(order, req.user.id);
  } catch (e) {
    console.error('Auto-deduct stok gagal:', e.message);
  }
  ...
  res.json({ status: 'ok', message, order: { ...updated, packed_by_name: req.user.full_name }, stock_warning: stockWarning });
});
